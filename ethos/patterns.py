...
def detect_patterns(agent_id: str) -> PatternResult:
    ...
    try:
        ...
        eval_count = get_agent_evaluation_count(service, agent_id)
        if eval_count < _MIN_EVALUATIONS:
            service.close()
            return PatternResult(agent_id=agent_id, checked_at=checked_at)

        # Get pattern definitions from graph (Pattern -> COMPOSED_OF -> Indicator)
        pattern_map = get_pattern_indicator_map(service)
        if not pattern_map:
            service.close()
            return PatternResult(agent_id=agent_id, checked_at=checked_at)

        # Get all indicators detected for this agent
        agent_indicators = get_agent_detected_indicators(service, agent_id)
